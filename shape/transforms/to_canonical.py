    ...
    def apply_transform(self, subject: tio.data.Subject) -> tio.data.Subject:
        ...
        if shapes_dict:
            if isinstance(self.shape_image_key, str):
                ...
                and len(
                    ShapeSupportSubject.exclude_shapes(
                        subject.get_images_dict(intensity_only=False)
                    )
                )
                == 1
            ):
                shape_image_key = list(
                    subject.get_images_dict(intensity_only=False).keys()
                )[0]
            else:
                raise ValueError(
            ...
            src_ref_shape = torch.tensor(
                deepcopy(subject[shape_image_key].spatial_shape)
            )
