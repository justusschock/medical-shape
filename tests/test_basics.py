...
def test_transforms(trafo):
    ...
    assert not isinstance(transformed_subject, ShapeSupportSubject) and isinstance(transformed_subject, tio.data.Subject)

def test_warning_wrong_subject_type():
    clear_warning_caches()
    trafo = ToCanonical()
    with pytest.warns(UserWarning, match="Using a shape transformation without an explicit ShapeSupportSubject may lead to unexpected behaviour"):
        # breakpoint()
...
def test_warning_wrong_trafo_type():
    ...
    with pytest.warns(UserWarning, match="Using the ShapeSupportSubjects together with one or more Shape instances and "
